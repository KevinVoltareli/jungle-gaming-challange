...
let NotificationTypeOrmRepository = class NotificationTypeOrmRepository {
    ...
    async save(notification) {
        const entity = notification_mapper_1.NotificationMapper.toOrm(notification);
    ...
    async findById(id) {
        ...
        if (!entity)
            return null;
        return notification_mapper_1.NotificationMapper.toDomain(entity);
    }
    async listByUser(userId, page, size) {
        ...
        return {
            items: rows.map(notification_mapper_1.NotificationMapper.toDomain),
            total,
        };
    }
};
